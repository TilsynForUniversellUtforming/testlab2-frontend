...
class TokenRelayInterceptor(
    ...
    override fun intercept(
        ...
        return if (authentication is OAuth2AuthenticationToken) {
                runCatching {
                    ...
                    val hasExpired = hasTokenExpired(client.accessToken)
                    logger.debug("Has expired: $hasExpired")
                    val refreshToken = client.refreshToken
                    println("Refreshtoken " +  refreshToken?.tokenValue)
    ...
    private fun loadClient(
        ...
        clientService.loadAuthorizedClient<OAuth2AuthorizedClient>(
            clientRegistrationId, authentication.name) ?: throw RuntimeException("Client not found")
