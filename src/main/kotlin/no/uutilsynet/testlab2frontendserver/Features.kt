...
class Features(val featuresProperties: FeaturesProperties) {
  ...
  fun list(): List<Feature> {
    return listOf(Feature("maalinger", featuresProperties.maalinger))
  }
}

data class Feature(val key: String, val active: Boolean)

@ConfigurationProperties(prefix = "features") data class FeaturesProperties(val maalinger: Boolean)
