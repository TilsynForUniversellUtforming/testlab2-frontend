...
import { Styringsdata, StyringsdataListElement, StyringsdataVerksemd, } from '@test/styringsdata/types';

const styringsdataApiBaseUrl = '/api/v1/styringsdata';

export const fetchStyringsdataVerksemd = async (
  styringsdataId: number
): Promise<StyringsdataVerksemd> => {
  return await fetch(`${styringsdataApiBaseUrl}/verksemd/${styringsdataId}`, {
    method: 'GET',
  }).then((response) =>
...
export const fetchStyringsdata = async (
  ...
  return await fetch(`${styringsdataApiBaseUrl}/loeysing/${styringsdataId}`, {
    method: 'GET',
  }).then((response) =>
    responseToJson(response, 'Kunne ikkje hente styringsdata')
  );
